...
All functions are pure (no side effects) and return pandas/numpy objects.
"""
...
def consecutive_pairs(df: pd.DataFrame, cols: List[str]) -> pd.Series:
    """Distribution of consecutive number pair count per draw."""
    def _count(row):
        nums = sorted(row.values)
        return sum(1 for a, b in zip(nums, nums[1:]) if b == a + 1)
    return df[cols].apply(_count, axis=1).value_counts().sort_index().rename('nb_tirages')
...
def temporal_frequency(
    ...
    """Frequency of each number grouped by time period (default: year-end)."""
    df2 = df.copy()
...
def companions(
    ...
    # Retard: draws since last appearance among the filtered draws only
