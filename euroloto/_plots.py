"""
Visualization functions — each returns a matplotlib Figure (no file saving).
Designed for inline display in Jupyter notebooks.
"""
...
def frequency_bar(
    df: pd.DataFrame, cols: List[str], config: dict, label: str = 'main'
) -> plt.Figure:
...
def cooccurrence_heatmap(
    df: pd.DataFrame, cols: List[str], config: dict,
    top_n: int = 20, min_cooc: int = 0,
) -> plt.Figure:
    ...
    sns.heatmap(
        sub, annot=True, fmt='.0f', cmap='YlOrRd', ax=ax,
...
def gap_boxplot(df: pd.DataFrame, cols: List[str], config: dict) -> plt.Figure:
    ...
    expected_gap = (n_max - n_min + 1) / len(cols)
...
def sum_histogram(df: pd.DataFrame, cols: List[str], config: dict) -> plt.Figure:
    ...
    ax.hist(sums, bins=45, density=True, color='#3498db', edgecolor='white',
...
def hot_cold_chart(
    df: pd.DataFrame, cols: List[str], config: dict, n_recent: int = 50
) -> plt.Figure:
...
def temporal_trend(
    df: pd.DataFrame, cols: List[str], config: dict, top_n: int = 10
) -> plt.Figure:
    """Normalized yearly frequency chart (appearances per 100 draws)."""
    date_col = config['date_col']
    draws_per_year = df.groupby(df[date_col].dt.year).size().rename('n_draws')
    ...
    for num in top_nums:
        if num in norm.columns:
            ax.plot(years_numeric, norm[num].values, marker='o', markersize=4,
                    label=str(num), alpha=0.85)

    for change_year, label in config.get('structural_changes', {}).items():
        ax.axvline(change_year - 0.5, color='#e74c3c', linestyle='--', linewidth=1.5, alpha=0.7)
...
def retard_bar(df: pd.DataFrame, cols: List[str], config: dict) -> plt.Figure:
    """Horizontal bar chart of draws since last appearance — top 20 most overdue."""
...
def companions_bar(
    ...
    """Bar chart of companion number frequencies for a single game."""
    if comp_freq.empty:
...
def companions_bar_all(
    ...
    """Stacked bar chart of companion frequencies combining Loto and EuroMillions."""
    top_nums = comp_all.index.tolist()
    loto_vals = comp_loto['frequence'].reindex(top_nums, fill_value=0) if not comp_loto.empty else pd.Series(0, index=top_nums)
    euro_vals = comp_euro['frequence'].reindex(top_nums, fill_value=0) if not comp_euro.empty else pd.Series(0, index=top_nums)

    x = np.arange(len(top_nums))
    width = 0.6

    fig, ax = plt.subplots(figsize=(max(12, len(top_nums) * 0.8), 6))
    ax.bar(x, loto_vals.values, width, color='#3498db', edgecolor='white', linewidth=0.4,
           label=f'Loto ({len(filtered_loto)} tirages)')
    ax.bar(x, euro_vals.values, width, bottom=loto_vals.values, color='#e74c3c',
           edgecolor='white', linewidth=0.4, label=f'EuroMillions ({len(filtered_euro)} tirages)')
